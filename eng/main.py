...
def get_external_links(text):
    infoe = re.compile("== ?External links ?==")
    m = infoe.search(text)
    if m:
...
def get_references(text):
    infor = re.compile("== ?(Notes|References|Citations|Bibliography) ?==")
    m = infor.search(text)
...
def sanitizeStop(vec, type_vec):
    ...
    x = []
    # return ["hi", "how", "you"]
    extra_stop = ["\"", ":", "[", "]", ".", "(", ")", "-", ",", "'", ";", "?", "user", "categori", "1", "0", "2", "3", "utc", "nowiki", "br", "delet"]
    if type_vec == "body" or type_vec == "references":
        extra_stop += citation_words
    if type_vec != "title" and type_vec != "categories":
        extra_stop += html_words


    stopers = set(extra_stop+stopedwords)
